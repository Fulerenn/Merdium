...
const command: MerdiumChatCommand = {
    name: "ban",
    description: "Give's a ban to a target user",
    ...
    run: (message: Message) => {
        ...
        if (target.id == message.author.id)
            return message.channel.send(
                `You can not ban yourself! \`\`${target.id} (${target.tag}) == ${message.author.id} (${message.author.tag})\`\``
            );
        ...
        if (!targetMember.bannable)
            return message.channel.send(
                `Could not ban this user! \`\`member.bannable == false\`\``
            );

        targetMember.ban({ reason: reason }).then((user: GuildMember) => {
            return message.channel.send(
                `Done! Banned user ${user.displayName} (${user.id}) for ${reason}`
            );
        });
    },
};
